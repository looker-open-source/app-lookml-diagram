...
import {DiagramField} from "../utils/diagrammer"
import {
  isTableRowDimension,
  isTableRowMeasure,
  isTableRowBaseView,
...
export function createLookmlViewElement(
  ...
  .attr("d", (dd: DiagramField, i: number) => i === 0 ? `M0,${TABLE_ROW_HEIGHT}
  ...
  .attr("d", (dd: DiagramField, i: number) => i === (tableData.length - 1) ? `M0,0
    h${TABLE_WIDTH}
    v${TABLE_ROW_HEIGHT - CAP_RADIUS}
    q0,${CAP_RADIUS} -${CAP_RADIUS},${CAP_RADIUS}
    h-${TABLE_WIDTH - (CAP_RADIUS * 2)}
